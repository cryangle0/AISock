...
import { parseSockGeometry, type SockGeometry } from '@/components/editor/sockVector'
...
const FALLBACK_SOCKS: EditorSock[] = SOCK_TYPES.map((s) => ({ id: s.id, name: s.name, desc: s.desc, family: null, _id: 0 }))

// ── 模块级缓存（跨页面共享，仅首次请求）──
const socks = ref<EditorSock[]>(FALLBACK_SOCKS)
const patterns = ref<EditorPattern[]>(BUILTIN_PATTERNS)
const geoCache = new Map<string, SockGeometry | null>()
let loaded = false
...
export function useCatalog() {
  ...
  /** 按袜型 code 拉取矢量几何（按需 + 缓存）；无几何返回 null（调用方回退参数化绘制） */
  async function getGeometry(code: string): Promise<SockGeometry | null> {
    if (geoCache.has(code)) return geoCache.get(code)!
    await ensureLoaded()
    const sock = socks.value.find((s) => s.id === code)
    if (!sock?._id) { geoCache.set(code, null); return null }
    try {
      const res = await catalogApi.getSock(sock._id)
      const geo = parseSockGeometry(res.data?.geometry_json)
  ...
  return { socks, patterns, defaultSockId: DEFAULT_SOCK_TYPE_ID, ensureLoaded, getGeometry }
}
